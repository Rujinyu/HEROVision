...
def cox_loss(hazard_scores, T, E):
    n = T.shape[0]
    hazard_scores = hazard_scores.view(-1)
    T = T.view(-1)
    E = E.view(-1)
    risk_set = torch.sum(E.float() * (T >= T).float(), dim=0)
    risk_set = torch.where(risk_set == 0, torch.ones_like(risk_set), risk_set)
    neg_partial_log_like = torch.sum(
        (hazard_scores * E - torch.log(torch.sum(torch.exp(hazard_scores) * (T >= T).float(), dim=0))))/ risk_set

    return neg_partial_log_like.mean()
...
class HoverTrans(nn.Module):
    def __init__(self, img_size=224, patch_size=32, in_chans=3, num_classes=2, embed_dim=768, dim=[4, 8, 16, 32], depth=[2, 4, 4, 2],
...
class HoverTrans_Stage2(nn.Module):
    def __init__(self, img_size=224, patch_size=32, in_chans=3, num_classes=2, embed_dim=768, dim=[4, 8], depth=[2,1],
                 num_heads=[2, 4], num_inner_head=[2, 4], mlp_ratio=4., drop_rate=0., attn_drop_rate=0.,
                 drop_path_rate=0.1, norm_layer=nn.LayerNorm):
        super().__init__()
        self.num_classes = num_classes
        self.embed_dim = embed_dim
...
class FourModelHover(nn.Module):
    def __init__(self,img_size=64,patch_size=8):
        super().__init__()
        self.P_Hover=HoverTrans_Stage2(img_size=128,patch_size=[16,8],num_classes=128)
        self.T2_Hover=HoverTrans_Stage2(img_size=128,patch_size=[16,8],num_classes=128)
        self.DWI_Hover = HoverTrans_Stage2(img_size=128,patch_size=[16,8],num_classes=128)
        self.CEUS_Hover = HoverTrans_Stage2(img_size=128,patch_size=[16,8],num_classes=128)
        ...
        self.t2_mlp=nn.Sequential(
            nn.Linear(384, 128),
            nn.BatchNorm1d(128),
            nn.ReLU(inplace=True),
        )
        self.dwi_mlp=nn.Sequential(
            nn.Linear(256, 128),
            nn.BatchNorm1d(128),
            nn.ReLU(inplace=True),
        )
        self.ceus_mlp=nn.Sequential(
            nn.Linear(384,128),
            nn.BatchNorm1d(128),
            nn.ReLU(inplace=True),
        )
    ...
    def forward(self,p_imgs,t2_imgs,dwi_imgs,ceus_imgs):
        p_result=[]
        t2_result=[]
        dwi_result=[]
        ceus_result=[]
        for i in range(p_imgs.shape[2]):
            p_result.append(self.P_Hover(p_imgs[:,:,i,:,:]))
        for i in range(t2_imgs.shape[2]):
            t2_result.append(self.T2_Hover(t2_imgs[:,:,i,:,:]))
        for i in range(dwi_imgs.shape[2]):
            dwi_result.append(self.DWI_Hover(dwi_imgs[:,:,i,:,:]))
        for i in range(ceus_imgs.shape[2]):
            ceus_result.append(self.CEUS_Hover(ceus_imgs[:,:,i,:,:]))
        p_fea=torch.concat(p_result,dim=1)
        t2_fea=torch.concat(t2_result,dim=1)
        dwi_fea=torch.concat(dwi_result,dim=1)
        ceus_fea=torch.concat(ceus_result,dim=1)
        final=[]
        final.append(self.p_mlp(p_fea))
        final.append(self.t2_mlp(t2_fea))
        final.append(self.dwi_mlp(dwi_fea))
        final.append(self.ceus_mlp(ceus_fea))
        final=torch.concat(final,dim=1)
        output=self.final_mlp(final)
        return output,final
